...
template <>
struct Traits<ARMstrongKDL::doubleVel> {
	typedef double valueType;
	typedef ARMstrongKDL::doubleVel derivType;
};
...
template<> struct std::hash<ARMstrongKDL::doubleVel>
{
    std::size_t operator()(ARMstrongKDL::doubleVel const& dv) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, dv.value());
        ARMstrongKDL::hash_combine(seed, dv.deriv());
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::VectorVel>
{
    std::size_t operator()(ARMstrongKDL::VectorVel const& vv) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, vv.p);
        ARMstrongKDL::hash_combine(seed, vv.v);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::RotationVel>
{
    std::size_t operator()(ARMstrongKDL::RotationVel const& rv) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, rv.R);
        ARMstrongKDL::hash_combine(seed, rv.w);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::FrameVel>
{
    std::size_t operator()(ARMstrongKDL::FrameVel const& fv) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, fv.M);
        ARMstrongKDL::hash_combine(seed, fv.p);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::TwistVel>
{
    std::size_t operator()(ARMstrongKDL::TwistVel const& tv) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, tv.vel);
        ARMstrongKDL::hash_combine(seed, tv.rot);
        return seed;
    }
};
