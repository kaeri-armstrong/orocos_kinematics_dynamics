...
template<> struct std::hash<ARMstrongKDL::doubleAcc>
{
    std::size_t operator()(ARMstrongKDL::doubleAcc const& da) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, da.t);
        ARMstrongKDL::hash_combine(seed, da.d);
        ARMstrongKDL::hash_combine(seed, da.dd);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::VectorAcc>
{
    std::size_t operator()(ARMstrongKDL::VectorAcc const& va) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, va.p);
        ARMstrongKDL::hash_combine(seed, va.v);
        ARMstrongKDL::hash_combine(seed, va.dv);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::RotationAcc>
{
    std::size_t operator()(ARMstrongKDL::RotationAcc const& ra) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, ra.R);
        ARMstrongKDL::hash_combine(seed, ra.w);
        ARMstrongKDL::hash_combine(seed, ra.dw);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::FrameAcc>
{
    std::size_t operator()(ARMstrongKDL::FrameAcc const& fa) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, fa.M);
        ARMstrongKDL::hash_combine(seed, fa.p);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::TwistAcc>
{
    std::size_t operator()(ARMstrongKDL::TwistAcc const& ta) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, ta.vel);
        ARMstrongKDL::hash_combine(seed, ta.rot);
        return seed;
    }
};
