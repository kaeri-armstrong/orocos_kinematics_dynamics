...
template<> struct std::hash<ARMstrongKDL::Vector>
{
    std::size_t operator()(ARMstrongKDL::Vector const& v) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, v.x());
        ARMstrongKDL::hash_combine(seed, v.y());
        ARMstrongKDL::hash_combine(seed, v.z());
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::Rotation>
{
    std::size_t operator()(ARMstrongKDL::Rotation const& r) const noexcept
    {
        size_t seed = 0;
        double x, y, z, w;
        r.GetQuaternion(x, y, z, w);
        ARMstrongKDL::hash_combine(seed, x);
        ARMstrongKDL::hash_combine(seed, y);
        ARMstrongKDL::hash_combine(seed, z);
        ARMstrongKDL::hash_combine(seed, w);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::Frame>
{
    std::size_t operator()(ARMstrongKDL::Frame const& f) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, f.p);
        ARMstrongKDL::hash_combine(seed, f.M);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::Wrench>
{
    std::size_t operator()(ARMstrongKDL::Wrench const& w) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, w.force);
        ARMstrongKDL::hash_combine(seed, w.torque);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::Twist>
{
    std::size_t operator()(ARMstrongKDL::Twist const& t) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, t.vel);
        ARMstrongKDL::hash_combine(seed, t.rot);
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::Vector2>
{
    std::size_t operator()(ARMstrongKDL::Vector2 const& v) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, v.x());
        ARMstrongKDL::hash_combine(seed, v.y());
...
template<> struct std::hash<ARMstrongKDL::Rotation2>
{
    std::size_t operator()(ARMstrongKDL::Rotation2 const& r) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, r.GetRot());
        return seed;
    }
};

template<> struct std::hash<ARMstrongKDL::Frame2>
{
    std::size_t operator()(ARMstrongKDL::Frame2 const& f) const noexcept
    {
        size_t seed = 0;
        ARMstrongKDL::hash_combine(seed, f.p);
        ARMstrongKDL::hash_combine(seed, f.M);
