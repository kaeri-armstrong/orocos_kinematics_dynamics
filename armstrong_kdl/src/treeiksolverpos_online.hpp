...
// This class has been derived from the ARMstrongKDL::TreeIkSolverPos_NR_JL class
 ...
 * Cartesian to joint space of a general ARMstrongKDL::Tree. This class has been derived from the TreeIkSolverPos_NR_JL class,
...
private:
  /**
   * Scales the class member ARMstrongKDL::JntArray q_dot_, if one (or more) joint velocity exceeds the maximum value.
   * Scaling is done proportional to the biggest overshoot among all joint velocities.
   */
  void enforceJointVelLimits();

  /**
   * Scales translational and rotational velocity vectors of the class member ARMstrongKDL::Twist twist_,
