...
void SolverTest::setUp()
{
    ...
    //  Segment Inertias
    ARMstrongKDL::RigidBodyInertia inert1(15.0, ARMstrongKDL::Vector(0.0, -0.02, 0.0),                       // mass, CM
                                 ARMstrongKDL::RotationalInertia(0.1, 0.05, 0.1, 0.0, 0.0, 0.0));   // inertia
    ARMstrongKDL::RigidBodyInertia inert2(5.0, ARMstrongKDL::Vector(0.0, -0.02, -0.1),
                                 ARMstrongKDL::RotationalInertia(0.01, 0.1, 0.1, 0.0, 0.0, 0.0));
    ARMstrongKDL::RigidBodyInertia inert3(5.0, ARMstrongKDL::Vector(0.0, -0.05, 0.02),
                                 ARMstrongKDL::RotationalInertia(0.05, 0.01, 0.05, 0.0, 0.0, 0.0));
    ARMstrongKDL::RigidBodyInertia inert4(3.0, ARMstrongKDL::Vector(0.0, 0.02, -0.15),
                                 ARMstrongKDL::RotationalInertia(0.1, 0.1, 0.01, 0.0, 0.0, 0.0));
    ARMstrongKDL::RigidBodyInertia inert5(3.0, ARMstrongKDL::Vector(0.0, -0.05, 0.01),
                                 ARMstrongKDL::RotationalInertia(0.02, 0.01, 0.02, 0.0, 0.0, 0.0));
    ARMstrongKDL::RigidBodyInertia inert6(3.0, ARMstrongKDL::Vector(0.0, -0.01, -0.1),
                                 ARMstrongKDL::RotationalInertia(0.1, 0.1, 0.01, 0.0, 0.0, 0.0));
    ARMstrongKDL::RigidBodyInertia inert7(1.0, ARMstrongKDL::Vector(0.0, 0.0, 0.05),
                                 ARMstrongKDL::RotationalInertia(0.01, 0.01, 0.1, 0.0, 0.0, 0.0));
...
void SolverTest::IkSingularValueTest()
{
	...
	dF.M = ARMstrongKDL::Rotation::RPY(0.1, 0.1, 0.1) ;
	dF.p = ARMstrongKDL::Vector(0.01,0.01,0.01) ;
	...
	F_error = ARMstrongKDL::diff(F_solved,F_des);
	...
	// big displacement
	dF.M = ARMstrongKDL::Rotation::RPY(0.2, 0.2, 0.2) ;
	dF.p = ARMstrongKDL::Vector(-0.2,-0.2, -0.2) ;
...
void SolverTest::IkVelSolverWDLSTest()
{
	...
	ARMstrongKDL::ChainIkSolverVel_wdls ikvelsolver(motomansia10,eps,maxiter) ;
	ikvelsolver.setLambda(lambda) ;
	unsigned int nj = motomansia10.getNrOfJoints();
    JntArray q(nj), dq(nj) ;

	ARMstrongKDL::Vector	v05(0.05,0.05,0.05) ;
	ARMstrongKDL::Twist dx(v05,v05) ;
...
void SolverTest::VereshchaginTest()
{
    ...
    ARMstrongKDL::JntArray q(nj); //input
    ARMstrongKDL::JntArray qd(nj); //input
    ARMstrongKDL::JntArray qdd(nj); //output
    ARMstrongKDL::JntArray ff_tau(nj); //input
    ARMstrongKDL::JntArray constraint_tau(nj); //output
    ...
    ARMstrongKDL::Vector f(10.0, 15.0, 0.0);
    ARMstrongKDL::Vector n(0.0, 0.0, 5.0);
    ARMstrongKDL::Wrench f_tool(f, n);
    ARMstrongKDL::Wrenches f_ext(ns);
...
void SolverTest::FdSolverDevelopmentTest()
{
    ...
    ChainDynParam DynSolver = ARMstrongKDL::ChainDynParam(motomansia10dyn, gravity);
    ...
    ARMstrongKDL::ChainIdSolver_RNE IdSolver = ARMstrongKDL::ChainIdSolver_RNE(motomansia10dyn, gravity);
...
void SolverTest::FdSolverConsistencyTest()
{
    ...
    Vector gravity(0.0, 0.0, -9.81);  // base frame
    ARMstrongKDL::ChainFdSolver_RNE FdSolver = ARMstrongKDL::ChainFdSolver_RNE(motomansia10dyn, gravity);

    unsigned int nj = motomansia10dyn.getNrOfJoints();
    unsigned int ns = motomansia10dyn.getNrOfSegments();

    // Joint position, velocity, and acceleration
    ARMstrongKDL::JntArray q(nj);
    ARMstrongKDL::JntArray qd(nj);
    ARMstrongKDL::JntArray qdd(nj);
    ARMstrongKDL::JntArray tau(nj);
    ...
    ARMstrongKDL::Vector f(10,-20,30) ;
    ARMstrongKDL::Vector n(3,-4,5) ;
    ARMstrongKDL::Wrench f_tool(f,n);
    // In local link coordinates
    ARMstrongKDL::Wrenches f_ext(ns);
    for(unsigned int i=0;i<ns;i++){
    ...
    ARMstrongKDL::ChainIdSolver_RNE IdSolver = ARMstrongKDL::ChainIdSolver_RNE(motomansia10dyn, gravity);
    ARMstrongKDL::JntArray torque(nj);
...
void SolverTest::FdAndVereshchaginSolversConsistencyTest()
{
    ...
    ARMstrongKDL::JntArray qd(nj);
    ARMstrongKDL::JntArray qdd(nj);
    ARMstrongKDL::JntArray ff_tau(nj);
    ...
    ARMstrongKDL::Vector f(10.0, -20.0, 30.0);
    ARMstrongKDL::Vector n(3.0, -4.0, 5.0);
    ARMstrongKDL::Wrench f_tool(f, n);
    ...
    ARMstrongKDL::ChainFdSolver_RNE FdSolver = ARMstrongKDL::ChainFdSolver_RNE(kukaLWR, gravity);
...
void SolverTest::ExternalWrenchEstimatorTest()
{
    ...
    ARMstrongKDL::ChainIdSolver_RNE IdSolver(kukaLWR, linearAcc);
    ...
    ARMstrongKDL::SetToZero(alpha); // Set to zero to deactivate all constraints
    ARMstrongKDL::SetToZero(beta); // Set to zero to deactivate all constraints
    ...
    // Prepare test cases
    std::vector<ARMstrongKDL::JntArray> jnt_pos;
    std::vector<ARMstrongKDL::Wrench> wrench_reference;
