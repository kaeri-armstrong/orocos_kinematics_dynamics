 ...
 */
void test_inverseposkin(ARMstrongKDL::Chain& chain) {
#if BOOST_VERSION < 108300
    boost::timer timer;
#else
    ...
    ARMstrongKDL::ChainFkSolverPos_recursive fwdkin(chain);
    ARMstrongKDL::ChainIkSolverPos_LMA solver(chain,L);
    ARMstrongKDL::JntArray q(n);
    ARMstrongKDL::JntArray q_init(n);
    ARMstrongKDL::JntArray q_sol(n);
    for (int trial=0;trial<num_of_trials;++trial) {
        q.data.setRandom();
        q.data *= ARMstrongKDL::PI;
        q_init.data.setRandom();
        q_init.data *= ARMstrongKDL::PI;
        ARMstrongKDL::Frame pos_goal,pos_reached;
...
int main(int argc,char* argv[]) {
    ...
    ARMstrongKDL::Chain chain;
    chain = ARMstrongKDL::Puma560();
    //chain = ARMstrongKDL::KukaLWR_DHnew();

    ARMstrongKDL::ChainIkSolverPos_LMA solver(chain);
